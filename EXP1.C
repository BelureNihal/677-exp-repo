...
// Performs a binary search on a sorted array to find the first occurrence
int search(int arr[], int size, int target) 
{
    int low = 0;
    int high = size - 1;
    int result_index = -1; // Stores the found index

    while (low <= high) {
        int mid = low + (high - low) / 2;

        if (arr[mid] == target) {
            result_index = mid; // Record the index
            high = mid - 1;     // Keep searching left for the 1st occurrence
        } 
...
// Counts total occurrences of the target in the array
int count(int arr[], int size, int target) 
{
    int occurrences = 0; // Initialize counter to 0
